...
proc corr data=outputs.firm_s_t_sort_ac cov vardef=weight out=outputs.m_v_ac noprint;
...
print col_names;
create outputs.test2_results_wdenom from results [colname=col_names];
append from results;
...
print col_names;
create outputs.residuals_wdenom from resids [colname=col_names]; 
append from resids;
close outputs.residuals;

quit;

proc export data=outputs.test2_results_wdenom outfile="&wrkpath./test2_results_wdenom.csv" DBMS=CSV replace; run;
proc export data=outputs.residuals_wdenom outfile="&wrkpath./residuals_wdenom.csv" DBMS=CSV replace; run;
